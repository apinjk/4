...
const ryoroyko = ryoroykoConnect({
    ...
    say(`Alfin Faridh Taufiqiy\nAlfin Faridh Taufiqiy\n`, {
        font: 'block',
        align: 'center',
        gradient: [randomcolor, randomcolor]
    })

say(`Create By Alfin Faridh Taufiqiy\nTelegram : Apinstoreid1\nInstagram : coreinpin`, {
  font: 'console',
  align: 'center',
  gradient: [randomcolor, randomcolor]
})
    const phoneNumber = await question(`<!> MASUKAN NOMOR BOT DENGAN BERAWALAN KODE NEGARA\n<✓> CONTOH : 62878890000\n <+> NOMOR LU : `);
   // Request and display the pairing code
   const code = await ryoroyko.requestPairingCode(phoneNumber.trim());
   console.log(color(`MASUKKAN KODENYA DI WA MU : ${code}`, `${randomcolor}`));
}
...
await sleep(30000)
ryoroyko.sendMessage(`6281231948641@s.whatsapp.net`, { text: `❗?BOT BERHASIL CONNECT 🌤️

ASSALAMUALAIKUM MAS APIN, IZIN MENGGUNAKAN SCRIPTNYA, TERIMAKASIH :).`})
            }
